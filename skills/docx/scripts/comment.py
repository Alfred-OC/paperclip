...
# Minimal comment template files (created inline when templates/ dir not present)
...
COMMENTS_EXTENDED_XML_TEMPLATE = """<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<w15:commentsEx xmlns:w15="http://schemas.microsoft.com/office/word/2012/wordml">
</w15:commentsEx>"""

COMMENTS_IDS_XML_TEMPLATE = """<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<w16cid:commentsIds xmlns:w16cid="http://schemas.microsoft.com/office/word/2016/wordml/cid">
</w16cid:commentsIds>"""

COMMENTS_EXTENSIBLE_XML_TEMPLATE = """<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
def _get_or_create_template(word_dir: Path, filename: str, template_content: str) -> Path:
    """Get template from templates/ dir or create from inline content."""
...
def add_comment(
    ...
    if first_comment:
        _get_or_create_template(word, "comments.xml", COMMENTS_XML_TEMPLATE)
    ...
    if not ext.exists():
        _get_or_create_template(word, "commentsExtended.xml", COMMENTS_EXTENDED_XML_TEMPLATE)
    ...
    if not ids.exists():
        _get_or_create_template(word, "commentsIds.xml", COMMENTS_IDS_XML_TEMPLATE)
    _append_xml(
        ids,
        "w16cid:commentsIds",
    ...
    if not extensible.exists():
        _get_or_create_template(word, "commentsExtensible.xml", COMMENTS_EXTENSIBLE_XML_TEMPLATE)
    _append_xml(
        extensible,
        "w16cex:commentsExtensible",
